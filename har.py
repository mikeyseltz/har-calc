...
class Engagement:
    ...
    def execute(self):
        ...
        self.update_state(c.time_to_act(self.state['delay'],
                                        self.state['vel']))
        self.update_state(c.first_90(self.state['vel'],
                                     self.state['g']))
        self.update_state(c.second_90(self.state['vel'],
                                      self.state['g'], dive))
        self.update_state(c.accel_in_dive(self.state['vel'],
                                          self.state['end_vel'],
                                          self.state['dive']))
        self.update_state(c.straight_dive(self.state['vel'],
                                          self.state['dive'],
                                          self.state['alt']))
        self.update_state(c.recover_to_level(self.state['vel'],
                                             self.state['dive'],
                                             self.state['alt']))
