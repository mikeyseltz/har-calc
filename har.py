...
class Engagement:
	...
	state = {'alt': None, 'vel':None, 'rng':None, 'dive':None, 'alive':True, 'time':0}
	...
	def execute(self, st_alt=30000, st_vel=400, dive=45, first_90_dive=0, end_vel=500):
		...
		self.state['alt'] = input("start altitude in feet >>> ")
		self.state['vel'] = input("start speed in knots >>> ")
		self.state['rng'] = input("starting range to tgt in nm >>> ")*6036
		dive = input("target dive angle >>> ")
		print(f'starting at {self.state['alt']}ft and {self.state['vel']} knots, diving to {dive} degrees')
		delay = input("how long to react in seconds >>> ")
		self.update_state(c.time_to_act(delay, self.state['vel']))

		g = input("how many gs will you pull? >>> ")
		self.update_state(c.first_90(self.state['vel'], g))
		self.update_state(c.second_90(self.state['vel'], dive))
		end_vel = input('what will you accelerate to in ktas? >>> ')
	...
	def update_state(self, deltas):
		...
		print(f'At time {self.state['time']}: {self.state['alt']}ft, \
		 Rng: {self.state['rng']/6036}nm, Dive: {self.state['dive']}deg')
